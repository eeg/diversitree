...
statecols <- c("AB"="purple", "A"="blue", "B"="red")
plot(geosse.phy, tip.color=statecols[geosse.phy$tip.state+1], cex=0.5, no.margin=TRUE)
@
\caption{
...
p <- coef(ml1)
lik1(p)
lik4 <- make.geosse(geosse.phy, geosse.phy$tip.state, sampling.f=c(0.9, 0.6, 0.4))
lik4(p)
@
...
Some applications of GeoSSE have combined multiple clades into a single analysis \citep{anacker2011, goldberg-inpress}.
...
The trees {\tt geosse.phy.cea} and {\tt geosse.phy.arc} and the tip states {\tt geosse.chars.cea} and {\tt geosse.chars.arc} (A = chaparral, B = forest) were read in with the call to {\tt data("geosse")} above.
...
(I actually used a somewhat messier procedure, but Rich suggested this {\tt combine()} function.)
<<chaplik2>>=
...
% find.mle(lnL.multi, chaparral.params)
%           sA           sB           xA           xB           dA           dB 
% 1.333745e-01 3.676180e-02 2.659455e-02 1.014985e-01 3.981056e-01 7.809753e-08
% [1] -382.0964
